...
/*******************************************************************************/
XFUNC int zlog_add_format(const char *name, const char *pattern)
{
	int rc = 0;
	int i;
	zlog_format_t *a_format = NULL;
	zlog_format_t *old_format = NULL;
	...
	a_format = zlog_format_new_from_parts(name, pattern,
			&(zlog_env_conf->time_cache_count));
	if (!a_format) {
		zc_error("zlog_format_new_from_parts fail");
		goto err;
	}

	/* check if format with same name already exists */
	for (i = 0; i < zc_arraylist_len(zlog_env_conf->formats); i++) {
		old_format = zc_arraylist_get(zlog_env_conf->formats, i);
		if (old_format && zlog_format_has_name(old_format, name)) {
			/* update all rules that reference the old format */
			int j;
			zlog_rule_t *a_rule;
			zc_arraylist_foreach(zlog_env_conf->rules, j, a_rule) {
				if (a_rule->format == old_format) {
					a_rule->format = a_format;
				}
			}
			/* replace in-place */
			zc_arraylist_set(zlog_env_conf->formats, i, a_format);
			a_format = NULL; /* transferred ownership */
			goto done;
		}
	}

	/* not found, add new */
	if (zc_arraylist_add(zlog_env_conf->formats, a_format)) {
	...
	return 0;
err:
	if (a_format) zlog_format_del(a_format);
	rc = pthread_rwlock_unlock(&zlog_env_lock);
	if (rc) {
		zc_error("pthread_rwlock_unlock fail, rc=[%d]", rc);
		return -1;
...
/*******************************************************************************/
XFUNC int zlog_add_rule(const char *rule_line)
{
	int rc = 0;
	int i = 0;
	int c_up = 0;
	zlog_rule_t *a_rule = NULL;
	char line_buf[MAXLEN_CFG_LINE + 1];

	zc_assert(rule_line, -1);
	...
	/* copy because zlog_rule_new may modify the line */
	snprintf(line_buf, sizeof(line_buf), "%s", rule_line);

	a_rule = zlog_rule_new(line_buf,
	...
	if (zc_arraylist_add(zlog_env_conf->rules, a_rule)) {
		zc_error("zc_arraylist_add fail");
		goto err;
	}
	a_rule = NULL; /* transferred ownership */

	/* update all categories to pick up the new rule */
	if (zlog_category_table_update_rules(zlog_env_categories,
			zlog_env_conf->rules)) {
		c_up = 0;
		zc_error("zlog_category_table_update_rules fail");
		goto err;
	} else {
		c_up = 1;
	}
...
err:
	...
	if (a_rule) zlog_rule_del(a_rule);
	rc = pthread_rwlock_unlock(&zlog_env_lock);
	if (rc) {
		zc_error("pthread_rwlock_unlock fail, rc=[%d]", rc);
		return -1;
...
/*******************************************************************************/
XFUNC int zlog_remove_rules(const char *category)
{
	int rc = 0;
	int i;
	int c_up = 0;
	...
	for (i = zc_arraylist_len(zlog_env_conf->rules) - 1; i >= 0; i--) {
		a_rule = zc_arraylist_get(zlog_env_conf->rules, i);
		if (a_rule && STRCMP(a_rule->category, ==, category)) {
			zc_arraylist_remove_keep_order(zlog_env_conf->rules, i);
			removed++;
		}
	}
...
/*******************************************************************************/
XFUNC int zlog_has_format(const char *name)
{
	int rc = 0;
	int i;
	int found = 0;
	...
	zc_arraylist_foreach(zlog_env_conf->formats, i, a_format) {
		if (a_format && zlog_format_has_name(a_format, name)) {
			found = 1;
			break;
		}
	}
...
/*******************************************************************************/
XFUNC int zlog_has_category_rules(const char *category)
{
	int rc = 0;
	int i;
	int found = 0;
